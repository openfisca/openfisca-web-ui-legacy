...
<%
    is_owner = legislation.is_owner(ctx)
    is_editable = is_owner and legislation.is_dated
%>\
...
title="${_(u'Duplicate legislation for today values')}">
    ...
    % endif
    % if is_owner:
                    <a class="btn btn-default" href="${legislation.get_user_url(ctx, 'edit')}">${_(u'Edit')}</a>
                    <a class="btn btn-danger"  href="${legislation.get_user_url(ctx, 'delete')}">${_(u'Delete')}</a>
    % endif
                </div>
            </div>
        </div>
...
%>\
    ...
    % else:
<%
        json_from = legislation.json.get('start') if legislation.json is not None else None
...
%>\
        ...
        % endif
<%
        json_to = legislation.json.get('stop') if legislation.json is not None else None
...
<%
    is_editable = legislation.is_owner(ctx) and legislation.is_dated
...
%>\
    % if value is not None:
        ...
        % if dated_legislation_json is not None:
                ${legislation_tree.render_dated_legislation_node(node = dated_legislation_json, is_editable = is_editable)}
        % elif value.get('datesim') is not None:
                ${legislation_tree.render_dated_legislation_node(node = value, is_editable = is_editable)}
        % else:
