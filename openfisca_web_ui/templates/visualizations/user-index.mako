...
<%def name="container_content()" filter="trim">
    ...
    % else:
        ...
        % for visualization in visualizations:
                <tr>
                    <td>
                        <h4><a href="${visualization.get_user_url(ctx)}">${visualization.title}</a></h4>
<%
...
%>\
            ...
            % endif
                    ...
                    <td>${visualization.updated.split('T')[0]}</td>
                </tr>
        % endfor
            </tbody>
        </table>
...
%>\
    % if user is not None and user.email is not None:
        <div class="btn-toolbar">
            <a class="btn btn-default" href="${model.Legislation.get_user_class_url(ctx, 'new')}">${_(u'New')}</a>
        </div>
    % endif
</%def>
